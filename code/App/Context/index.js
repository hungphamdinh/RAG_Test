...
export const useHandlerAction = () => {
  ...
  const createRequestHandler = (showLoading, showToast) => (type, fnc) =>
    processRequest(dispatch)(type, fnc, { showLoading, showToast });

  const withLoadingAndErrorHandling = createRequestHandler(true);
  const withLoadingAndToastHandling = createRequestHandler(true, true);
  const withErrorHandling = createRequestHandler(false);
  return {
    withLoadingAndErrorHandling,
    withErrorHandling,
    withLoadingAndToastHandling,
  };
};
