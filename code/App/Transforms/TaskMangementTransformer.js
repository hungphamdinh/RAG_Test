...
export const transformMentionUsers = (data) => {
  ...
  return _.uniqBy(transformedData, 'id');
};
...
export const convertRemindersToReminder = (reminders) => {
  ...
  return {
    isActive: true,
    reminderInMinutes: reminders.map((item) => item.reminderInMinute).filter((item) => item !== undefined),
    reminderInMinutesAfter: reminders.map((item) => item.reminderInMinuteAfter).filter((item) => item !== undefined),
...
export function getCacheKey(json) {
