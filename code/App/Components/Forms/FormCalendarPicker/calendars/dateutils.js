...
function month(xd) {
  ...
  const monthData = xd.getMonth();
  const days = new Date(year, monthData + 1, 0).getDate();

  const firstDay = new XDate(year, monthData, 1, 0, 0, 0, true);
  const lastDay = new XDate(year, monthData, days, 0, 0, 0, true);
