...
const TaskDetail = ({ navigation }) => {
  const {
    taskManagement: { priorityList, statusList, teamsTaskDetail, usersInTeams, taskDetail, employeesByTenant, tenantsDetail },
    getTaskDetail,
    addTask,
    updateTask,
    getUsersInTeamByTenants,
    getTeamsForTaskDetail,
    resetTaskDetail,
    getEmployeesByTenant,
    getTenantsTaskDetail,
  } = useTaskManagement();

  const {
    user: { user, isControlOffice },
  } = useUser();
  ...
  const [teamIds, assignees, reminder, tenant, parentId] = watch([
    'teamIds',
    'assignees',
    'reminder',
    'tenant',
  ...
  useEffect(() => {
    if (reminder?.isActive && _.size(assignees)) {
      const reminderUsers = reminder?.users || [];
      const userIds = _.uniq([...assignees.map((item) => item.id, user.id, ...reminderUsers.map((item) => item.id))]);
      const users = _.uniqBy([...assignees, user, ...reminderUsers], 'id');
  ...
  const onChangeTenantId = (selectedTenant) => {
    if (isTeamSelectionFirst) {
      getTeamsForTaskDetail({ tenantId: selectedTenant.id, isCO: isPublic });
      return;
    }
  ...
  const resetTeamAssignee = (isTeamChoose, isCO = isPublic) => {
    ...
    setValue('assignees', []);
    if (isTeamChoose) {
      getTeamsForTaskDetail({ tenantId: tenant.id, isCO });
      return;
    }
  ...
  const handleTeamChange = async (selectedTeams) => {
    ...
    const isCOTeam = (teamId) => {
      let team = _.find(teamsTaskDetail, (team) => team.id === teamId);

      // Team not return in teamList but still have data in taskDetail
      if (!team && taskDetail.teams) {
        team = _.find(taskDetail.teams, (team) => team.id === teamId);
      }
  ...
  const goToTask = (item, isSubTask) => {
    navigation.replace(isSubTask ? 'editSubTask' : 'editTask', {
      id: item.id,
    });
  };
  ...
  const handleSave = async (values) => {
    ...
    const transformedTask = transformParams(payload, isTeamSelectionFirst ? teamsTaskDetail : teamsByUsers, id);
  ...
  return (
    <BaseLayout {...mainLayoutProps}>
        ...
        <AwareScrollView>
          {!isAddNew && taskDetail?.subTasks && (
            <TaskLinkages
              maxVisibleItems={5}
              text="SUB_TASK"
              linkages={taskDetail?.subTasks}
              onPress={(item) => goToTask(item, true)}
            />
          )}
          {!isAddNew && parentId && <TaskLinkages text="TASK" linkages={[{ id: parentId }]} onPress={goToTask} />}
          <FormProvider {...formMethods}>
            ...
            <FormLazyDropdown
              listExist={tenantsDetail.data}
              showSearchBar
              getList={(_page, keyword) =>
                getTenantsTaskDetail({
                  filter: keyword,
                })
              }
              testID="project"
              options={tenantsDetail}
              title="PROJECT"
              label="PROJECT"
              name="tenant"
              ...
              <RowWrapper>
                <DateWrapper>
                  <FormDate
                    ...
                    small
                    mode="datetime"
                    minimumDate={watch('taskStartDate')}
      ...
      {!isAddNew && <TaskManagementComment teamList={teamsTaskDetail} task={watch()} />}
    </BaseLayout>
  );
};
