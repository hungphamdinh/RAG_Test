...
const AttachImages = () => {
  ...
  const onImageChanged = (newImage) => {
    ...
    if (submitOnline) {
      const imageGuid = newImage.files.guid;
      newEditedImage.imageGuid = imageGuid;
      newEditedImage.id = imageGuid; // use for Draggable list
    }
...
const onDeleteSelectedItem = () => {
  ...
  const onBtAddPress = () => {
    NavigationServices.navigate('inspectionCamera', { onCompleted: handleAddImage, isRequiredLocation, images, moduleId });
  };
