...
const renderComponent = (isHistory) =>
  renderScreen(<MaintenancePlan navigation={{ navigate: mockNavigate }} isHistory={isHistory} />)();
...
describe('MaintenancePlan Component', () => {
  ...
  it('calls getAssetsPlan on mount with correct parameters for Maintenance History', async () => {
    ...
    const expectedParams = {
      ...
      page: 1,
      pageSize: 20,
      statusIds: [PM_STATUS_ID.CLOSED],
    };

    expect(mockGetAssetsPlan).toHaveBeenCalledWith(expectedParams);
  });

  it('changes selected index and fetches data when a segment is pressed for Maintenance PLan', async () => {
